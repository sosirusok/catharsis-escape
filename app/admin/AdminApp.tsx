...
export default function AdminApp() {
  ...
  const activeReservations = data.reservations.filter((item) => item.status === "confirmed" || item.status === "checked_in");
  const guestTotal = activeReservations.reduce((sum, item) => sum + Number(item.party_size), 0);

  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        <a className="admin-logo" href={publicSiteUrl()}><span>C</span><div><strong>카타르시스</strong><small>RESERVATION ADMIN</small></div></a>
...
function Dashboard({ date, setDate, reservations, guestTotal, onNew, onOpen }: { date: string; setDate: (value: string) => void; reservations: ReservationRow[]; guestTotal: number; onNew: () => void; onOpen: (value: ReservationRow) => void }) {
  const active = reservations.filter((item) => item.status === "confirmed" || item.status === "checked_in");
  return <div className="admin-content"><div className="admin-heading-row"><DateToolbar date={date} setDate={setDate} /><button className="admin-primary" onClick={onNew}>＋ 예약 직접 추가</button></div><div className="metric-grid"><Metric label="오늘 예약" value={`${active.length}건`} note="확정 및 입장 예약" /><Metric label="방문 인원" value={`${guestTotal}명`} note="예약 기준 합계" /><Metric label="취소" value={`${reservations.filter((item) => item.status === "cancelled").length}건`} note="오늘 취소된 예약" /><Metric label="운영 테마" value={`${new Set(active.map((item) => item.theme_id)).size}개`} note="예약이 있는 테마" /></div><section className="admin-panel"><div className="panel-title"><div><p>TODAY</p><h2>오늘 일정</h2></div><span>{active.length ? `${minuteToTime(active[0]?.start_minute)}부터` : "예정된 예약 없음"}</span></div><ReservationList reservations={reservations} onOpen={onOpen} /></section></div>;
}
...
function ReservationEditor({ value, themes, date, onClose, onSaved }: { value: ReservationRow | "new"; themes: ThemeRow[]; date: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="admin-modal-backdrop" onMouseDown={onClose}><form className="admin-modal" onSubmit={save} onMouseDown={(event) => event.stopPropagation()}><button type="button" className="admin-modal-close" onClick={onClose}>×</button><p className="eyebrow">{current ? String(current.booking_code) : "NEW RESERVATION"}</p><h2>{current ? "예약 상세" : "예약 직접 추가"}</h2><div className="admin-form-grid"><label>테마<select value={form.themeId} onChange={(event) => setForm({ ...form, themeId: event.target.value })} disabled={Boolean(current)}>{themes.filter((item) => item.status !== "archived").map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.name)}</option>)}</select></label><label>날짜<input type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} disabled={Boolean(current)} /></label><label>시간<input type="time" value={form.time} onChange={(event) => setForm({ ...form, time: event.target.value })} disabled={Boolean(current)} /></label><label>인원<input type="number" min="1" max="30" value={form.partySize} onChange={(event) => setForm({ ...form, partySize: Number(event.target.value) })} /></label><label>대표자 이름<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>전화번호<input type="tel" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label>{current && <label>상태<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}><option value="confirmed">예약 확정</option><option value="checked_in">입장</option><option value="completed">이용 완료</option><option value="cancelled">예약 취소</option><option value="no_show">미방문</option></select></label>}<label className="full">관리자 메모<textarea value={form.memo} onChange={(event) => setForm({ ...form, memo: event.target.value })} placeholder="직원끼리 확인할 내용을 입력하세요." /></label></div>{error && <div className="admin-form-error">{error}</div>}<div className="admin-modal-actions"><button type="button" className="admin-secondary" onClick={onClose}>닫기</button><button className="admin-primary" disabled={saving}>{saving ? "저장 중…" : current ? "변경사항 저장" : "예약 등록"}</button></div></form></div>;
}
...
function Themes({ themes, onSaved }: { themes: ThemeRow[]; onSaved: () => void }) {
  ...
  return <div className="admin-content"><div className="admin-heading-row"><div className="admin-section-copy"><h2>테마 목록</h2><p>노출 정보와 예약 조건을 한곳에서 관리합니다.</p></div><button className="admin-primary" onClick={() => setEditing({ ...blankTheme })}>＋ 새 테마 추가</button></div><div className="theme-admin-grid">{themes.filter((item) => item.status !== "archived").map((item) => <article className="theme-admin-card" key={String(item.id)}><div className={`theme-admin-art ${String(item.art_key)}`} style={item.image_key ? { backgroundImage: `url(${themeImageUrl(String(item.image_key))})` } : undefined}><span className={`visibility ${String(item.status)}`}>{item.status === "active" ? "공개" : "숨김"}</span></div><div><p>{String(item.genre)}</p><h3>{String(item.name)}</h3><dl><div><dt>난이도</dt><dd>{String(item.difficulty)} / 5</dd></div><div><dt>진행</dt><dd>{String(item.duration_min)}분</dd></div><div><dt>인원</dt><dd>{String(item.min_people)}–{String(item.max_people)}인</dd></div></dl><button onClick={() => setEditing(item)}>테마 수정</button></div></article>)}</div>{editing && <ThemeEditor value={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); onSaved(); }} />}</div>;
}
...
function ThemeEditor({ value, onClose, onSaved }: { value: ThemeRow; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="admin-modal-backdrop" onMouseDown={onClose}><form className="admin-modal theme-editor" onSubmit={save} onMouseDown={(event) => event.stopPropagation()}><button type="button" className="admin-modal-close" onClick={onClose}>×</button><p className="eyebrow">THEME EDITOR</p><h2>{value.id ? "테마 수정" : "새 테마 추가"}</h2><div className="theme-editor-layout"><div className="admin-form-grid"><label className="full">공개 상태<select value={form.status} onChange={(event) => set("status", event.target.value)}><option value="active">공개</option><option value="hidden">숨김</option></select></label><label className="full">테마명<input value={form.name} onChange={(event) => set("name", event.target.value)} /></label><label>예약용 짧은 이름<input value={form.shortName} onChange={(event) => set("shortName", event.target.value)} /></label><label>장르<input value={form.genre} onChange={(event) => set("genre", event.target.value)} /></label><label className="full">줄거리<textarea value={form.synopsis} onChange={(event) => set("synopsis", event.target.value)} /></label><label>기본 이미지<select value={form.artKey} onChange={(event) => set("artKey", event.target.value)}><option value="life">인생테마 이미지</option><option value="office">사무실 이미지</option><option value="knock">잠입 이미지</option></select></label><label>대표 이미지 업로드<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => upload(event.target.files?.[0])} /></label><label>난이도<input type="number" min="1" max="5" value={form.difficulty} onChange={(event) => set("difficulty", Number(event.target.value))} /></label><label>진행 시간<input type="number" min="30" max="180" value={form.durationMin} onChange={(event) => set("durationMin", Number(event.target.value))} /></label><label>정리 시간<input type="number" min="0" max="180" value={form.turnoverMin} onChange={(event) => set("turnoverMin", Number(event.target.value))} /></label><label>최소 인원<input type="number" min="1" max="20" value={form.minPeople} onChange={(event) => set("minPeople", Number(event.target.value))} /></label><label>최대 인원<input type="number" min="1" max="30" value={form.maxPeople} onChange={(event) => set("maxPeople", Number(event.target.value))} /></label><label className="full">주의사항<input value={form.notice} onChange={(event) => set("notice", event.target.value)} /></label><div className="full price-editor"><span>인원별 팀 요금</span>{Array.from({ length: Math.max(0, form.maxPeople - form.minPeople + 1) }, (_, index) => form.minPeople + index).map((people) => <label key={people}>{people}인<input type="number" step="1000" value={form.prices[String(people)] || 0} onChange={(event) => set("prices", { ...form.prices, [String(people)]: Number(event.target.value) })} /></label>)}</div></div><div className="theme-live-preview"><span>고객 화면 미리보기</span><div className={`preview-art ${form.artKey}`} style={form.imageKey ? { backgroundImage: `url(${themeImageUrl(form.imageKey)})` } : undefined} /><p>{form.genre || "장르"} · {form.durationMin}분</p><h3>{form.name || "테마명"}</h3><div className="preview-difficulty">{Array.from({ length: 5 }, (_, index) => <i className={index < form.difficulty ? "on" : ""} key={index} />)}</div><small>{form.minPeople}–{form.maxPeople}인</small></div></div>{error && <div className="admin-form-error">{error}</div>}<div className="admin-modal-actions">{value.id && <button type="button" className="admin-danger-link" onClick={archive}>운영 종료</button>}<button type="button" className="admin-secondary" onClick={onClose}>취소</button><button className="admin-primary" disabled={saving}>{saving ? "저장 중…" : value.id ? "변경사항 저장" : "테마 등록"}</button></div></form></div>;
}
...
function Settings({ settings, onSaved }: { settings: SettingsRow; onSaved: () => void }) {
  ...
  return <div className="admin-content"><form className="admin-panel settings-form" onSubmit={save}><div className="panel-title"><div><p>BOOKING POLICY</p><h2>예약 설정</h2></div><button className="admin-primary" disabled={saving}>{saving ? "저장 중…" : "설정 저장"}</button></div><div className="booking-switch"><div><strong>온라인 예약 접수</strong><p>끄면 고객 화면의 모든 예약 시간이 닫힙니다.</p></div><button type="button" className={form.bookingOpen ? "on" : ""} onClick={() => setForm({ ...form, bookingOpen: !form.bookingOpen })}><span /></button></div><div className="settings-grid"><label>예약 공개 기간<span>오늘부터 예약 가능한 날짜 범위</span><div className="unit-input"><input type="number" min="1" max="31" value={form.horizonDays} onChange={(event) => setForm({ ...form, horizonDays: Number(event.target.value) })} /><em>일</em></div></label><label>당일 예약 마감<span>게임 시작 전 접수 마감 시간</span><div className="unit-input"><input type="number" min="0" max="1440" step="10" value={form.leadMinutes} onChange={(event) => setForm({ ...form, leadMinutes: Number(event.target.value) })} /><em>분 전</em></div></label><label>고객 취소 마감<span>이 시간이 지나면 전화 취소만 가능</span><div className="unit-input"><input type="number" min="0" max="10080" step="60" value={form.cancelCutoffMinutes} onChange={(event) => setForm({ ...form, cancelCutoffMinutes: Number(event.target.value) })} /><em>분 전</em></div></label><label>매장 전화번호<span>예약 완료와 취소 안내에 표시</span><input value={form.storePhone} onChange={(event) => setForm({ ...form, storePhone: event.target.value })} /></label><label className="full">예약 중지 안내 문구<span>예약 접수를 껐을 때 고객에게 표시</span><input value={form.pausedMessage} onChange={(event) => setForm({ ...form, pausedMessage: event.target.value })} /></label></div>{error && <div className="admin-form-error">{error}</div>}</form></div>;
}
