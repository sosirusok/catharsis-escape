...
function PrivacyPolicy({ settings }: { settings: PolicySettings }) {
  ...
  return (
    <>
      ...
      <section><h2>3. 처리업무의 위탁</h2><ul><li><strong>네이버파이낸셜 주식회사</strong>: 네이버페이 결제 승인, 결제취소·환불과 부정거래 방지</li><li><strong>토스페이먼츠 주식회사</strong>: 카드결제 승인, 결제취소·환불과 분쟁 처리</li><li><strong>Cloudflare, Inc.</strong>: 서버·데이터베이스 운영, 콘텐츠 전송, 접속 보안과 장애 대응</li></ul><p>실제 결제 화면에 표시되는 결제기관이 해당 거래를 처리합니다. 수탁자가 개인정보를 목적 외로 처리하지 않도록 관계 법령과 계약에 따라 관리합니다.</p></section>
      ...
      <section><h2>5. 안전성 확보조치</h2><p>개인정보 전송 구간을 암호화하고 관리자 접근을 제한합니다.</p></section>
...
function TermsPolicy({ settings }: { settings: PolicySettings }) {
  return (
    <>
      <section><h2>제1조 목적</h2><p>이 약관은 {settings.businessName || "카타르시스 이스케이프"}(이하 “매장”)이 제공하는 방탈출 테마 예약, 온라인 결제, 예약조회와 취소 서비스의 이용조건 및 매장과 고객의 권리·의무를 정합니다.</p></section>
      <section><h2>제2조 예약 신청과 성립</h2><p>고객은 테마, 날짜, 시간, 인원, 예약자 이름과 휴대전화번호를 정확히 입력하고 결제 전에 예약 내용과 최종 결제금액, 취소·환불 조건을 확인해야 합니다. 예약은 선택한 결제기관의 결제가 최종 승인되고 사이트가 예약번호를 발급한 때 확정됩니다.</p><p>화면에 예약 가능 시간이 표시되어도 다른 고객의 결제가 먼저 완료되면 해당 시간은 마감될 수 있습니다. 결제 전 임시 확보된 시간은 정해진 결제시간 안에 승인이 끝나지 않으면 자동으로 해제됩니다.</p></section>
      <section><h2>제3조 결제와 예약 확인</h2><p>결제 화면에 표시되는 카드 또는 네이버페이 등록 카드·계좌·포인트·머니로 결제할 수 있습니다. 결제가 완료되면 결제완료 화면과 예약조회에서 예약번호, 테마, 이용일시, 인원, 결제금액과 상태를 확인할 수 있습니다.</p><p>결제는 승인되었으나 예약번호가 발급되지 않은 경우 매장은 결제 상태를 확인하여 예약을 복구하거나 결제를 취소합니다.</p></section>
      <section><h2>제4조 변경·취소·환불</h2><p>예약 변경, 취소와 환불에는 결제할 때 확인한 취소·환불정책이 적용됩니다. 날짜·시간·테마 변경은 기존 예약 취소 후 새 예약으로 처리될 수 있습니다. 환불은 원래 결제수단으로 처리하며 반영 시점은 카드사, 은행 등 결제수단에 따라 달라질 수 있습니다.</p></section>
      <section><h2>제5조 고객의 의무와 현장 이용</h2><p>고객은 본인 또는 적법하게 사용할 권한이 있는 결제수단을 이용해야 하며 다른 사람의 정보나 결제수단을 도용해서는 안 됩니다. 테마 시작 10분 전까지 도착해 주세요. 지각하면 이용 시간이 줄어들거나 진행이 제한될 수 있습니다.</p></section>
      <section><h2>제6조 미성년자</h2><p>미성년자가 법정대리인의 동의 없이 계약한 경우 본인 또는 법정대리인은 관계 법령에 따라 계약을 취소할 수 있습니다. 만 14세 미만 고객의 온라인 예약은 법정대리인이 진행해야 합니다. 테마별 연령 및 보호자 동반 기준이 있는 경우 해당 안내가 우선합니다.</p></section>
      <section><h2>제7조 서비스 중단과 분쟁 해결</h2><p>천재지변, 정전, 통신망 또는 결제기관 장애, 긴급 안전점검으로 서비스가 중단될 수 있습니다. 이미 결제된 서비스를 제공할 수 없는 경우 일정 변경 또는 환불에 필요한 조치를 합니다. 분쟁은 상호 협의를 우선하고, 해결되지 않으면 한국소비자원 피해구제 또는 소비자분쟁조정 절차를 이용할 수 있습니다.</p></section>
      <section><h2>제8조 사업자 정보</h2><MerchantList settings={settings} /></section>
    </>
  );
}
...
function RefundPolicy({ settings }: { settings: PolicySettings }) {
  ...
  return (
    <>
      ...
      <section><h2>4. 취소 신청과 환불 처리</h2><p>예약조회에서 예약번호와 휴대전화번호를 입력해 신청하거나 아래 연락처로 문의할 수 있습니다. 환불은 원래 사용한 결제수단을 취소하는 방식으로 처리합니다. 취소 완료 뒤 한도 복원이나 계좌·포인트 반영까지 걸리는 시간은 결제수단에 따라 달라질 수 있습니다.</p><ContactLine settings={settings} /></section>
      <section><h2>5. 결제 오류</h2><p>이중결제, 승인금액 오류 또는 결제 후 예약 확정 실패가 확인되면 잘못된 결제를 전액 취소합니다. 결제완료 화면과 예약조회에서 결제금액과 예약 상태를 확인할 수 있습니다.</p></section>
